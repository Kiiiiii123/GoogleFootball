    ...
    def learn(self):
        ...
        for iteration in range(iter_num):
            ...
            if iteration % self.args.display_interval == 0:
                self.logger.info('[{}] update: {} / {}, Frames: {}, Rewards: {:.3f}, Max: {:.3f}, Min: {:.3f}: Policy ' \
                                 'Loss: {:.3f}, Value Loss: {:.3f}, Entropy Loss: {:.3f}'.format(datetime.now(), \
                                iteration, iter_num, (iteration + 1)*self.args.num_workers*self.args.nsteps, \
                                final_rewards.mean().item(), final_rewards.max().item(), final_rewards.min().item(), \
